...
function ScreenStart({ state, set, onContinue, onBack }) {
  ...
  const [discovered, setDiscovered] = React.useState(state.people.length > 0);
  ...
  const startScan = () => {
    ...
    setTimeout(() => {
      ...
      set({ people: picked, nfcCount: count, nearbyJoined: true });
      setDiscovered(true);
      setScanning(false);
    }, delay);
  };
      ...
      <div className="ss-screen-scroll">
        ...
        {discovered && (
          <div style={{ marginTop: 18, animation: 'fadeUp 0.4s ease' }}>
            <div className="eyebrow" style={{ marginBottom: 10 }}>{state.people.filter(p => p.trim()).length} joined</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {state.people.map((p, i) => {
                const isManual = i >= (state.nfcCount || 0);
                return (
                  <div key={i} className="item-row" style={{ animation: `fadeUp 0.4s ease ${i * 0.08}s both` }}>
                    <Avatar name={p || '?'} idx={i} />
                    {isManual ? (
                      <input
                        value={p}
...
function Radar({ scanning, discovered, people, onTap }) {
  return (
    <div onClick={!scanning && !discovered ? onTap : undefined} style={{
      ...
      {/* Discovered avatars positioned around */}
      {discovered && people.map((p, i) => {
        const angle = (i / people.length) * Math.PI * 2 + Math.PI / 4;
        const radius = 78;
        return (
          <div key={p} style={{
            position: 'absolute', top: '50%', left: '50%',
      ...
      <div style={{
        ...
        {scanning ? 'Scanning nearby devices…' : discovered ? `${people.length} people joined via NFC` : 'Tap to discover nearby people'}
      </div>
    </div>
  );
}
